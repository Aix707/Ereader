...
Ereader 是一款 Windows 本地阅读器 MVP，使用 Electron、React、Vite 和 TypeScript 构建。它面向本地小说与漫画阅读，支持导入 `txt`、`pdf`、`epub`、`mobi` 文件和图片文件夹，并把处理后的可阅读内容存入 SQLite 数据库。

## 功能概览

- 书架管理：导入文件、导入图片文件夹、搜索、最近阅读、继续阅读、删除与重建。
- 小说阅读：支持 TXT 段落阅读、EPUB/MOBI 文本与插图阅读、PDF 页阅读。
- 漫画阅读：支持图片文件夹、漫画 PDF、漫画 EPUB、图片主导 MOBI，提供单页/双页、左右阅读方向、滚轮翻页、页码跳转和相邻页预加载。
...
| MOBI | 支持 | 支持 | 未加密 PalmDOC/MOBI 可提取文本或图片页；AZW/AZW3 按 MOBI 入口尝试导入。 |
| PDF | 支持 | 支持 | 导入时高倍率渲染为 WebP 页面资产。 |
| 图片文件夹 | 不适用 | 支持 | 支持常见图片格式，自然数字排序。 |

PDF、EPUB 和 MOBI 可以在书架或阅读页切换小说/漫画内容类型。
